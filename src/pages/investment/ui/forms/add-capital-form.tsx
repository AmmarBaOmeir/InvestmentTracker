...
import {
  addCapitalToInvestment,
  buildCreateCapitalPayload,
} from "@/entities/asset/api/capitals";
...
export function AddCapitalForm({
  isOpen,
  onClose,
  investmentId,
  investment,
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const fullPayload = buildCreateCapitalPayload(formData, investmentId);
      const { investmentId: omittedId, ...payload } = fullPayload;
      void omittedId;
      await addCapitalToInvestment(investment, payload);
      resetForm();
      onSuccess?.();
      onClose();
    } catch (error) {
  ...
  return (
    <Modal
      ...
      footer={null}
    >
      <p className={styles.description}>{t("investment.add_capital_desc")}</p>
      {errorMessage ? (
